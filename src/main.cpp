...
void LoadMyAppSettings(AppState &appState) //
{
  appState.myAppSettings =
      StringToMyAppSettings(HelloImGui::LoadUserPref("UserInfo"));
}
void SaveMyAppSettings(const AppState &appState) {
  HelloImGui::SaveUserPref("UserInfo",
...
void DemoShowAdditionalWindow() {
  // Notes:
  //     - it is *not* possible to modify the content of the vector
  //     runnerParams.dockingParams.dockableWindows
  //       from the code inside a window's `GuiFunction` (since this GuiFunction
  //       will be called while iterating on this vector!)
  //     - there are two ways to dynamically add windows:
  //           * either make them initially invisible, and exclude them from the
  //           view menu (such as shown here)
  //           * or modify runnerParams.dockingParams.dockableWindows inside the
  //           callback RunnerCallbacks.PreNewFrame
  const char *windowName = "Additional Window";
  ImGui::PushFont(gTitleFont);
  ImGui::Text("Dynamically add window");
  ImGui::PopFont();
  if (ImGui::Button("Show additional window")) {
    auto additionalWindowPtr =
        HelloImGui::GetRunnerParams()->dockingParams.dockableWindowOfName(
            windowName);
...
void DemoUserSettings(AppState &appState) {
  ...
  ImGui::PopFont();
  ImGui::TextWrapped("The values below are stored in the application settings "
...
std::vector<HelloImGui::DockingSplit> CreateDefaultDockingSplits() {
  ...
  // Otherwise, modifications to the layout applied by the user layout will be
  // remembered. runnerParams.dockingParams.layoutCondition =
...
CreateDockableWindows(AppState &appState) {
  // A Command panel named "Commands" will be placed in "CommandSpace". Its Gui
  ...
  // additionalWindow is initially not visible (and not mentioned in the view
  // menu). it will be opened only if the user chooses to display it
...
int main(int, char **) {
  ...
  // Note: by setting the window title, we also set the name of the ini files in
  // which the settings for the user layout will be stored: Docking_demo.ini
  ...
  // Menu bar
  //
  runnerParams.imGuiWindowParams.showMenuBar =
      true; // We use the default menu of Hello ImGui
  runnerParams.callbacks.ShowMenus =
      ShowMenuGui; // where we add items to the default menu
  runnerParams.callbacks.ShowAppMenuItems =
      ShowAppMenuItems; // and where add items to the App menu

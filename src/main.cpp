...
void gui() {
  ...
  for (auto &x : level_ptr->coords()) {
    ...
    if (solved) {
      draw_list->AddNgonFilled(ImVec2(pixels.x, pixels.y), 20, HEX_REVEALED_COL,
                               6);
      ...
      draw_list->AddNgonFilled(ImVec2(pixels.x, pixels.y), 20, HEX_HIDDEN_COL,
                               6);
    }
  }
  ...
  if (is_active &&
      ImGui::IsMouseDragging(ImGuiMouseButton_Left, mouse_threshold_for_pan)) {
